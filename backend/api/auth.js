...
module.exports = app => {
    const signin = async (req,res) => {
        ...
        if(!usuario) return res.status(400).send('Usuario nao encontrado') //verifica se esse usuario realmente existe

        const isMatch = bcrypt.compareSync(req.body.password, usuario.password) //compara se a senha passada eh igual a senha do usuario no banco
        ...
        if(!isMatch) return res.status(401).send('Email ou senha invalidos')

        const now = Math.flooor(Date.now()/1000) //Date.now() me da os milisegundos desde primeiro de janeiro de 1970, entao dividindo por 1000 me da os segundos
        ...
        const payload = {
            ...
            exp: now + (60 * 60 * 24) //data de expiracao eh 60 segundos * 60 * 24, da um total de 1 dia a partir do IAT
        }
